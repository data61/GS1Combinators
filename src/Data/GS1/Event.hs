...
instance HasDWhat (Eventish a) where
  dWhat =
    lens
    (\(Eventish _ w _ _ _) -> w)
    (\(Eventish t _ w2 w3 w4) w1 -> Eventish t w1 w2 w3 w4)

instance HasDWhen (Eventish a) where
  dWhen =
    lens
    (\(Eventish _ _ w _ _ ) -> w)
    (\(Eventish t w1 _ w3 w4) w2 -> Eventish t w1 w2 w3 w4)

instance HasDWhy (Eventish a) where
  dWhy =
    lens
    (\(Eventish _ _ _ w _) -> w)
    (\(Eventish t w1 w2 _ w4) w3 -> Eventish t w1 w2 w3 w4)

instance HasDWhere (Eventish a) where
  dWhere =
    lens
    (\(Eventish _ _ _ _ w) -> w)
    (\(Eventish t w1 w2 w3 _) w4 -> Eventish t w1 w2 w3 w4)

newtype Event = Event (Eventish EventType)
  deriving (Show, Eq, Generic)

mkEvent :: EventType -> DWhat -> DWhen -> DWhy -> DWhere -> Maybe Event
mkEvent t w1 w2 w3 w4 = let e = (Just . Event) $ Eventish t w1 w2 w3 w4 in
                            case (t, w1) of
